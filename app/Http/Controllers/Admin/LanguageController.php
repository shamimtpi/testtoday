...
class LanguageController extends Controller
{
	...
	public function status($status,$id,$sid) {
	 ...
	 DB::update('update codepopular_langs set lang_status="'.$sid.'" where id = ?',[$id]);
	 return back();
	 }
	 ...
	 DB::update('update codepopular_langs set lang_default="'.$sid.'" where id = ?',[$id]);
	  $language = DB::table('codepopular_langs')
	              ->where('id','!=',$id)
		           ->get();
	  foreach($language as $newlangs)
	  {
	      DB::update('update codepopular_langs set lang_default="0" where id = ?',[$newlangs->id]);
	  }			   
	...
	public function destroy($id) {
		...
		$image = DB::table('codepopular_langs')->where('id', $id)->first();
		$orginalfile=$image->lang_flag;
		$photo="/media/language/";
       $path = base_path('images'.$photo.$orginalfile);
	  File::delete($path);
      DB::delete('delete from codepopular_langs where id!=2 and id = ?',[$id]);
