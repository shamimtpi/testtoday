...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
	...
	public function avigher_comment_reply(Request $request)
	{
		...
		{
		$commentitem = DB::table('product_comments')
						->where('comm_user_id','=',$comm_user_id)
						->where('item_id','=',$item_id)
						->get();
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
		 ...
		 $check = DB::table('product_refunds')
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
		...
		DB::insert('insert into product_comments (item_id,item_token,comm_user_id,item_user_id,comm_text,comm_date,comm_group_id,comm_parent) values (?,?,?, ?,?,?,?, ?)', [$item_id,$item_token,$comm_user_id, $item_user_id,$comm_text,$comm_date,$rand_id,$comm_id]);
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
		...
		$check_report = DB::table('product_reports')
	              ->where('item_id','=',$item_id)
		...
		{
		DB::insert('insert into product_reports (item_id,item_user_id,report_user_id,report_category,reason_for_report) values (?,?,?, ?,?)', [$item_id,$item_user_id,$report_user_id,$report_category,$report_message]);
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
	...
	$check_checkout = DB::table('product_checkouts')
	                  ->where('token','=',$token)
	...
	{
		DB::insert('insert into product_checkouts (
	...
	DB::update('update product_checkouts set 
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
		...
		if(empty($check))
		{
		    DB::insert('insert into product_ratings (user_id,item_id,rating,review,review_date) values (?, ?, ?, ?, ?)', [$user_id,$item_id,$rating,$review,$today]);
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }
	  ...
	  $logged = Auth::user()->id;
	  $get_viewer = DB::table('product_likes')
		            ->where('item_id','=',$id)
		...
		if(empty($get_viewer))
		{
			...
			DB::insert('insert into product_likes (item_id,user_id) values (?, ?)', [$id,$logged]);
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }			
	...
	$framework_count = DB::table('product_categories')
	   ...
	   /* item meta */
		...
		/*$check_item_free = DB::table('product_metas')
		        ->where('item_token', '=' , $token)
		...
		if(!empty($check_item_free))
		{
		    ...
		    $item_meta_wells = DB::table('product_metas')
		        ->where('item_token', '=' , $token)
				->where('item_meta_key', '=' , "item_type")
...
if(!empty(Cookie::get('lang'))){ $lang = Cookie::get('lang'); } else { if(!empty($default_cnt)){ $lang = $default[0]->lang_code; } else { $lang = "en"; } }	
	...
	$checkout_detail = DB::table('product_checkouts')
...
if(!empty($default_cnt)){ 
			$lang = $default[0]->lang_code;
			 } else { 
		    $lang = "en"; 
	     } 
	}
	...
	/*$category = DB::table('product_categories')
	  ...
	  $picture = '';
		...
		/*if($item_type!="")
		{
		   ...
		   $item_type_meta =  DB::table('product_metas')
		        				->where('item_token', '=' , $token)
				                ->where('user_id', '=' , $userid)
								->where('item_meta_key', '=' , 'item_type')
		                        ->count();
			...
			{
			   DB::update('update product_metas set item_meta_value="'.$item_type.'" where item_meta_key="item_type" and user_id="'.$userid.'" and item_token = ?', [$token]);
			}
			else
			{
			DB::insert('insert into product_metas (item_token,user_id,item_meta_key,item_meta_value) values (?, ?, ?, ?)', [$token,$userid,'item_type',$item_type]);
	...
	/*$category = DB::table('product_categories')
	  ...
	  /* graphic */
	   ...
	   $image = $request->file('preview_image');
		...
		if($site_watermark_status == "on")
			{
			$img = Image::make($url.'/local/images/media/preview/'.$filename)->encode('jpg', 50);
			 $img->insert($url.'/local/images/media/settings/'.$site_watermark,'center');
...
item_token,parent,item_type,item_script_type,item_layered,graphics_files,adobe_cs_version,pixel_dimensions,print_dimensions) values (?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?, ?,?,?,?,?,?,?, ?,?,?,?,?,?)', [$userid,$pagenamo,$pther,htmlentities($pagedeso),htmlentities($pagesort),$regular_price_six_month,  $regular_price_one_year,$extended_price_six_month,$extended_price_one_year,$high_resolution,$compatiblebrowser,  $file_included,$demo_url,$support_item,$future_update,$unlimited_download,  $categoryid,$frameworkid,$update_date,$savefname,$save_thumb,$zipname,$item_tags,$status_approval,$code,$token,$item_id,$item_type,$item_script_type,$item_layered,$graphicsid,$adobe_cs_version,$pixel_dimensions,$print_dimensions]);
	  ...
	  $picture = '';
		...
		/*if($item_type!="")
		{
		   ...
		   $check_item_key =  DB::table('product_metas')
		        				->where('item_token', '=' , $token)
			...
			{
			   DB::update('update product_metas set item_meta_value="'.$item_type.'" where item_meta_key="item_type" and user_id="'.$userridd.'" and item_token = ?', [$token]);
			}
			else
			{
			DB::insert('insert into product_metas (item_token,user_id,item_meta_key,item_meta_value) values (?, ?, ?, ?)', [$token,$userridd,'item_type',$item_type]);
		...
		if($videofile!="")
		{
		   ...
		   $check_item_meta =  DB::table('product_metas')
			...
			if(!empty($check_item_meta))
			{
			   DB::update('update product_metas set item_meta_value="'.$videoname.'" where item_meta_key="item_video_preview" and user_id="'.$userridd.'" and item_token = ?', [$token]);
			}
			else
			{
			DB::insert('insert into product_metas (item_token,user_id,item_meta_key,item_meta_value) values (?, ?, ?, ?)', [$token,$userridd,'item_video_preview',$videoname]);
